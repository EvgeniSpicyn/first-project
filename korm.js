function getProducts() {
    const products = [
        { subcategory: "Корм для хомячков", category: "Товары для животных", animal: "Хомячки", price: 7.99, inStock: true },
        { subcategory: "Корм для хомячков", category: "Товары для животных", animal: "Хомячки", price: 5.99, inStock: true },
        { subcategory: "Домик для хомячков", category: "Товары для животных", animal: "Хомячки", price: 12.99, inStock: true },
        { subcategory: "Колесо для хомячков", category: "Товары для животных", animal: "Хомячки", price: 7.49, inStock: false },
        { subcategory: "Клетка для хомячков", category: "Товары для животных", animal: "Хомячки", price: 29.99, inStock: true },
        { subcategory: "Наполнитель для клетки хомячков", category: "Товары для животных", animal: "Хомячки", price: 3.99, inStock: true },
        ...
        { subcategory: "Игрушки для собак", category: "Товары для животных", animal: "Собаки", price: 10.99, inStock: true }
    ];
    return products;
}
...
/* function filterProductsBy(products, filter){
    ...
    products.forEach(product => {
        let add = true;
        if(filter.category){
            add = product.category === filter.category
        }
        if(add && filter.animal){
            add = product.animal === filter.animal
        }
        if(add && filter.subcategory){
            add = product.subcategory === filter.subcategory
        }
        if(add){
            res.push(product)
        }
